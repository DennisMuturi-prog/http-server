...
use single_threaded_server::{
    chunked_parsing::{response_from_reader}, request_parser::Request, response_writer::{Response, ResponseWriter}, server::{Server, StatusCode}
};
...
fn connect_to_remote()->IoResult<()>{
    ...
    let response=response_from_reader(&mut connection);
